...
`common` 모듈의 `FeignHeaderPropagationInterceptor`가 모든 Feign 요청에 게이트웨이 헤더를 자동으로 전파합니다.
FeignClient에서 `@RequestHeader`로 헤더를 직접 선언할 필요가 없습니다.
...
`common` 모듈의 `CommonFeignErrorDecoder`가 자동 등록되어 Feign 오류 응답을 `RemoteServiceException`으로 변환합니다.
`RemoteServiceException`은 `BusinessException`을 상속하므로 `GlobalExceptionHandler`가 자동으로 처리합니다.
서비스 코드에서 Feign 예외를 직접 catch할 필요가 없습니다.

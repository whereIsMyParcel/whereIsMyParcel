...
상품 검증은 주문 저장 이전에 수행되는 조회성 작업입니다.
...
아직 외부 상태 변경이 없으므로 보상 트랜잭션이 필요하지 않습니다.

### 2.2 재고 예약 실패

주문은 `PENDING`으로 저장되었지만 재고 예약이 실패한 경우입니다.
...
이미 성공한 재고 예약을 취소해 정합성을 회복했으므로 최종 상태는 `FAILED`입니다.
...
Saga 구현 방식은 크게 Choreography와 Orchestration으로 나눌 수 있습니다.
...
따라서 다음 이유로 Orchestration을 선택했습니다.
...
- 단계가 재고 예약, 배송 생성, 재고 예약 취소 보상으로 단순함
...
중요한 점은 `createOrder()`가 전체 트랜잭션을 가지지 않는다는 것입니다.
...
`OrderCreateSagaContext`는 Saga 실행에 필요한 데이터를 담습니다.
...
현재 주문 생성 Saga에서 구현된 보상은 재고 예약 취소입니다.

배송 생성이 실패하면 Shipment Service에는 배송이 생성되지 않았거나 실패로 끝났다고 보고, 이미 성공한 Company Service 재고 예약을 취소합니다.

보상 요청은 다음 내부 API를 사용합니다.
...
이 경우 외부 서비스 응답이 지연되면 Order DB 트랜잭션과 커넥션이 오래 유지됩니다. 서킷브레이커가 있더라도 끊기 전까지의 대기 시간 동안 커넥션 점유는 줄어들지 않습니다.
...
Saga 테스트는 더 이상 `Order` 엔티티 상태를 직접 검증하지 않습니다. 대신 `OrderCreationStateService` 호출을 검증합니다.
...
### 11.1 배송 생성 성공 후 markConfirmed 실패

현재 Saga는 배송 생성 성공 후 `markConfirmed(orderId)`를 호출합니다. 만약 배송 생성은 성공했지만 Order DB 상태 저장이 실패하면, 배송은 생성되었지만 주문은 `STOCK_RESERVED`에 머물 수 있습니다.

이 경우 배송 취소 보상까지 자동으로 수행할지에 대한 정책은 별도 설계가 필요합니다. 현재 구현은 튜터 피드백에서 지적된 Feign 호출 중 DB 커넥션 점유와 재고 보상 실패 추적에 초점을 맞춥니다.

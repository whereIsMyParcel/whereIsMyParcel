...
public interface UserFeignClient {
     ...
     */
    @GetMapping("/internal/v1/users/by-business-number/{businessNumber}/id")
    ApiResponse<UserIdResponse> getUserIdByBusinessNumber(@PathVariable("businessNumber") String businessNumber);
    ...
    ApiResponse<Void> deleteUserOrClearCompany(@PathVariable("userId") UUID userId);

    /**
     * ④ [업체 전체 삭제 시] 해당 업체를 바라보는 모든 유저 일괄 연쇄 삭제 요청
     */
    @DeleteMapping("/internal/v1/users/companies/{companyId}")
    ApiResponse<Void> deleteAllUsersInCompany(@PathVariable("companyId") UUID companyId);
}

...
- **인접 리스트 최적화**: 매 루프마다 전체 경로를 필터링하는 대신, 인접 리스트(`Map<UUID, List<HubRoute>>`)를 미리 구성하여 시간 복잡도를 O(V+E) 수준으로 최적화했습니다.
- **데이터 직접 추출**: 다익스트라 완료 후 이미 계산된 `distances`, `durations` 맵에서 총합을 직접 추출하여 불필요한 리스트 순회 계산을 제거했습니다.

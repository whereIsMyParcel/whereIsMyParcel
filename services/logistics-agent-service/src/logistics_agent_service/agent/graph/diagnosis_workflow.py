...
class LangGraphDiagnosisWorkflow:
    ...
    normalize_input -> resolve_order -> collect_context -> diagnose -> generate_report
    """
    ...
    def run(self, query: DiagnosisQuery) -> DiagnosisResult:
        final_state = self._graph.invoke({"message": query.message})
        context = final_state.get("order_context")
